    ...
    private void Log(LogMsgType msgtype, string msg)
    {
      rtfTerminal.Invoke(new EventHandler(delegate
      {
          ...
          else
          {
              ...
              OdbcConnection conn0 = new OdbcConnection();
              String myConnectionString0 = "Driver={Microsoft Access Driver (*.mdb)};" + @"Dbq=C:\Users\water resources\Desktop\collegeDB.mdb";
              conn0.ConnectionString = myConnectionString0;
              conn0.Open();
              
              String getcostquery = "select cost from UserData where rfid = '" + msg + "';";

              OdbcCommand com1 = new OdbcCommand(getcostquery, conn0);
            OdbcDataReader cost_reader = com1.ExecuteReader();
            cost_reader.Read();
            
               String thisusercost = cost_reader[0].ToString();
               conn0.Close();
              ...
              String myQuery = "INSERT INTO rfid_log (inrfid,intime,cost) VALUES (?,?,?)";
              OdbcCommand cmd = new OdbcCommand(myQuery, conn);
              cmd.Parameters.Add("inrfid", OdbcType.VarChar).Value = Convert.ToString(msg);
              String nowDate = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
              cmd.Parameters.Add("intime", OdbcType.VarChar).Value = nowDate;
               
              cmd.Parameters.Add("cost", OdbcType.VarChar).Value = thisusercost;
              cmd.ExecuteNonQuery();
              conn.Close();
              rtfTerminal.AppendText(msg + " " + "Added to DB " + nowDate + " Cost: " + thisusercost + "\n");
